...
sudo service hhvm stop >/dev/null 2>&1
sudo killall hhvm >/dev/null 2>&1
...
sudo update-rc.d -f hhvm remove || { echo -e $RED'Warning! update-rc.d remove failed.'$ENDCOLOR ; }
sudo rm /etc/init.d/hhvm || { echo -e $RED'Warning! Removing init script failed.'$ENDCOLOR ; }
